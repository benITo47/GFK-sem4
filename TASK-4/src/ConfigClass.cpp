...
ConfigClass::ConfigClass(GUIMyFrame1 *wnd)
{
 ...
 dX=0;
 dY=0;
 show_markers = true;       //dodana funkcjonalnosc - moj kod
}
...
void ConfigClass::Save(char *filename)
{
 ...
 f << F_type << endl;
 f << show_markers << endl; //dodana funkcjonalnosc - moj kod
 f.close();
}
...
void ConfigClass::Load(char *filename)
{
 ...
 f >> F_type >> rd2EOL;
 f >> show_markers >> show_markers; //dodana funkcjonalnosc - moj kod
 f.close();
